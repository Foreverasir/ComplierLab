...
#define checkIfInt() do{\
	...
	else if(k!=INT){expInit();printf("Error type 12 at Line %d: What inside the \"[ ]\" is not an integer.\n",head->lineNum);return;}\
}while(0)
...
void expLBExpRB(Node* head,valueKind *expValKind,Type_ **expType){
	...
	if(h->childNum!=1 || h->child[0]->T!=ID_){
		expInit();
...
void semanticExtDef(Node *head){
	if(head->child[1]->T==FunDec){
		...
		if(f!=NULL)printf("Error type 4 at Line %d: Redefined function \"%s\".\n",head->lineNum,name);
		else{
			f=newFunc(name);
			addFunc(f);
			sta.currentFunc=f;
...
void semanticStructSpecifier(Node *head){
	if(head->childNum==2){
		...
		char name[32];
		if(head->child[1]->child[0]->T==None)
			getName(name);
		else
...
void semanticDec(Node *head){
	...
	if(head->childNum==3){
		if(sta.structFlag>0)
			...
			Type_ *t;
			semanticExp(head->child[2],&vkind,&t);
			if(!(t==NULL && vkind==USERDEF) && sta.currentValue!=NULL)
